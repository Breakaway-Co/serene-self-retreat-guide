-- Generate UUIDs and populate retreat configurations from the application data
DO $$
DECLARE
    stress_recovery_id UUID := gen_random_uuid();
    anxiety_relief_id UUID := gen_random_uuid();
    depression_support_id UUID := gen_random_uuid();
    ptsd_recovery_id UUID := gen_random_uuid();
    corporate_burnout_id UUID := gen_random_uuid();
    grief_healing_id UUID := gen_random_uuid();
BEGIN

-- Insert retreat configurations with generated UUIDs
...
-- Comprehensive daily content for Stress Recovery Retreat (7 days, 2 sessions per day)
...
-- Continue with all 7 days (2 sessions each = 14 total)
...
-- Add comprehensive content for Anxiety Relief Retreat (sample - full 20 sessions would continue)
INSERT INTO public.retreat_activities (
  retreat_id, day_number, time_slot, activity_name, activity_type,
  description, instructions, duration_minutes, evidence_base,
  trauma_considerations, difficulty_level
) VALUES
(anxiety_relief_id, 1, 'morning', 'Anxiety Assessment & Psychoeducation', 'reflection',
 ...
 'beginner'),
(anxiety_relief_id, 1, 'evening', 'Basic Anxiety Breathing Techniques', 'mindfulness',
...
-- Add comprehensive content for Depression Support Retreat (sample - full 28 sessions would continue)  
INSERT INTO public.retreat_activities (
  retreat_id, day_number, time_slot, activity_name, activity_type,
  description, instructions, duration_minutes, evidence_base,
  trauma_considerations, difficulty_level
) VALUES
(depression_support_id, 1, 'morning', 'Depression Assessment & Hope Building', 'reflection',
 ...
 'beginner'),
(depression_support_id, 1, 'evening', 'Gentle Mindfulness Introduction', 'mindfulness',
...
-- Add trigger to ensure data quality for evidence-based content
